#!/usr/bin/env python3
"""Materialize IC2 Task 5B as a 1.18.2 headless-server datapack.
...
DEFAULT_PATCH = SCENARIO_DIR / "ic2_task5a_converted_patch_1182.json"
DEFAULT_SERVER_DIR = PROJECT_ROOT / "headless_server/1.18.2"
DEFAULT_BASE_WORLD = DEFAULT_SERVER_DIR / "world"
DEFAULT_TARGET_WORLD = DEFAULT_SERVER_DIR / "world_ic2_task5b"
DEFAULT_REPORT = SCENARIO_DIR / "ic2_task5b_headless_materialization_report.json"
DEFAULT_SERVER_PROPERTIES = SCENARIO_DIR / "server_ic2_task5b.properties"
...
# Blocks that require missing mods on the headless server
UNAVAILABLE_MOD_BLOCKS = set()


def is_block_available(block_id: str) -> bool:
    return not any(block_id.startswith(prefix) for prefix in UNAVAILABLE_MOD_BLOCKS)


def fallback_block_id(block_id: str) -> str:
    if block_id.startswith("ftbic:"):
        return "conversion_placeholders:block_entity_placeholder"
    return block_id


def sanitize_nbt_for_indreb(nbt: dict[str, Any] | None) -> dict[str, Any] | None:
    """Remove inventory/battery/upgrade NBT for indreb blocks that can crash
    the headless server when ItemStackHandler Size mismatches the block's
    expected slot count.  The server will initialise empty handlers.
    FTBIC and other mods use their own inventory formats and are left intact.
    """
    if not nbt:
        return None
    cleaned = dict(nbt)
    for key in ("inventory", "battery", "upgrade"):
...
def setblock_command(edit: dict[str, Any]) -> str:
    ...
    if not is_block_available(block_id):
        ...
        nbt = edit.get("nbt")
        if block_id.startswith("indreb:"):
            nbt = sanitize_nbt_for_indreb(nbt)
        snbt = to_snbt(nbt) if nbt else ""
...
def generate_datapack(patch: dict[str, Any], datapack_dir: Path) -> tuple[list[str], dict[str, Any]]:
    ...
    pack_mcmeta = {
        "pack": {
            "pack_format": 9,
            "description": "IC2 Task 5B - materialize converted blocks",
        }
    }
    (datapack_dir / "pack.mcmeta").write_text(json.dumps(pack_mcmeta, indent=2) + "\n", encoding="utf-8")

    functions_dir = datapack_dir / "data" / "ic2_task5b" / "functions"
    ...
    (load_tag_dir / "load.json").write_text(
        json.dumps({"values": ["ic2_task5b:apply"]}, indent=2) + "\n", encoding="utf-8"
    )
    ...
    commands.append('say [IC2_TASK5B] apply complete')
...
def materialize(
    ...
    # Clean and copy base world
    if target_world.exists():
        shutil.rmtree(target_world)
    shutil.copytree(base_world, target_world)

    datapack_dir = target_world / "datapacks" / "ic2_task5b"
    commands, stats = generate_datapack(patch, datapack_dir)

    # Write server.properties
    server_properties = f"""# IC2 Task 5B server properties
level-name=world_ic2_task5b
server-port=25569
rcon.port=25579
...
online-mode=false
spawn-protection=0
motd=IC2 Task 5B Materialization
"""
    ...
    if existing_props.exists():
        backup = server_dir / "server.properties.before_ic2_task5b"
    ...
    print(f"Commands: {stats['commands']} (blocks={stats['block_edits']}, nbt={stats['nbt_edits']})")
