#!/usr/bin/env python3
"""Convert the materialized AE2 Task 5A 1.7.10 test world to a 1.18.2 patch."""
...
DEFAULT_WORLD = PROJECT_ROOT / "lightweigh_map_templates" / "1710_modded" / "ae2_task5a_world"
DEFAULT_SOURCE_PATCH = SCENARIO_DIR / "ae2_task5a_source_patch_1710.json"
DEFAULT_EXPECTED_PATCH = SCENARIO_DIR / "ae2_task5a_converted_patch_1182.json"
DEFAULT_OUT_PATCH = SCENARIO_DIR / "ae2_task5a_realworld_converted_patch_1182.json"
DEFAULT_REPORT = SCENARIO_DIR / "ae2_task5a_realworld_conversion_report.json"
...
def normalize_patch(patch: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for edit in patch.get("edits", []):
        item = {"op": edit.get("op"), "x": edit.get("x"), "y": edit.get("y"), "z": edit.get("z")}
        if edit.get("op") == "set_block":
            item["block_id"] = edit.get("block_id")
            item["properties"] = edit.get("properties") or {}
        elif edit.get("op") == "set_te":
            item["nbt"] = normalize_nbt(edit.get("nbt") or {})
        normalized.append(item)
    return sorted(normalized, key=lambda item: (item["x"], item["y"], item["z"], item["op"], item.get("block_id", "")))


def normalize_nbt(value: Any) -> Any:
    if isinstance(value, dict):
        normalized = {key: normalize_nbt(inner) for key, inner in value.items()}
        if "hasRedstone" in normalized and isinstance(normalized["hasRedstone"], bool):
            normalized["hasRedstone"] = int(normalized["hasRedstone"])
        if isinstance(normalized.get("parts"), list):
            normalized["parts"] = sorted(normalized["parts"], key=lambda item: str(item.get("part_id", "")))
        return normalized
    if isinstance(value, list):
        return [normalize_nbt(item) for item in value]
    return value
...
def convert_world(world: Path, source_patch_path: Path, expected_patch_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    source_patch = load_json(source_patch_path)
    expected_patch = load_json(expected_patch_path)
    ...
    for edit in source_blocks:
        ...
        if numeric_id != int(edit["id"]) or metadata != int(edit.get("meta", 0)):
            read_failures.append(
                f"Block mismatch for {edit.get('label')} at {(x, y, z)}: "
                f"got {numeric_id}:{metadata}, expected {edit['id']}:{edit.get('meta', 0)}"
            )
            continue
        pos = (x, y, z)
        nbt = find_te(chunk, x, y, z) if pos in expected_tes else None
        if pos in expected_tes and nbt is None:
            read_failures.append(f"Missing TE for {edit.get('label')} at {pos}")
            continue

        result = converter.convert_block(str(edit["registry_name"]), nbt, metadata, pos)
        converted = result.converted
        sample_results.append(
            {
                "label": edit.get("label"),
                "source": {"numeric_id": numeric_id, "registry_name": edit["registry_name"], "metadata": metadata, "position": [x, y, z], "te_id": nbt.get("id") if nbt else None},
        ...
        if not converted.success or not converted.block_id_1182:
            continue
        target_edits.append({"op": "set_block", "x": x, "y": y, "z": z, "block_id": converted.block_id_1182, "properties": converted.blockstate_props, "label": edit.get("label")})
        if converted.nbt_1182 is not None:
            target_edits.append({"op": "set_te", "x": x, "y": y, "z": z, "nbt": converted.nbt_1182, "label": edit.get("label")})
        for index, extra in enumerate(converted.additional_blocks):
            ex, ey, ez = extra.original_pos
            target_edits.append({"op": "set_block", "x": ex, "y": ey, "z": ez, "block_id": extra.converted.block_id_1182, "properties": extra.converted.blockstate_props, "label": f"{edit.get('label')}_additional_{index}"})
            if extra.converted.nbt_1182 is not None:
                target_edits.append({"op": "set_te", "x": ex, "y": ey, "z": ez, "nbt": extra.converted.nbt_1182, "label": f"{edit.get('label')}_additional_{index}"})

    target_patch = {
        "format_version": "1.18.2",
        "metadata": {"name": "ae2_task5a_realworld_converted", "generated_by": "convert_applied_test_world.py", "source_world": str(world)},
        "edits": target_edits,
    }
    patch_mismatches = compare_patches(target_patch, expected_patch)
    report = {
        "name": "AE2 Task 5A real-world conversion report",
...
def main() -> int:
    ...
    parser.add_argument("--out-patch", type=Path, default=DEFAULT_OUT_PATCH)
    parser.add_argument("--report", type=Path, default=DEFAULT_REPORT)
    args = parser.parse_args()

    patch, report = convert_world(args.world, args.source_patch, args.expected_patch)
    write_json(args.out_patch, patch)
