#!/usr/bin/env python3
"""Run Reliquary Task 6 headless tick/restart verification.

Sequence:
  1. Install server.properties with world_reliquary_task5b
  2. Start Forge 1.18.2 headless server
  3. Apply reliquary_task5b datapack
  4. Check all 14 converted blocks via RCON
...
DEFAULT_REPORT = SCENARIO_DIR / "reliquary_task6_headless_tick_restart_report.json"
SERVER_PROPERTIES_TEMPLATE = SCENARIO_DIR / "server_reliquary_task5b.properties"
REPORT_MD = SCENARIO_DIR / "RELIQUARY_TASK6_REPORT.md"
HANDOFF = SCENARIO_DIR / "HANDOFF_RELIQUARY_TASK6.md"

sys.path.insert(0, str(PROJECT_ROOT))
from src.converters.projectred.test_structures.headless_test.rcon_client import RconClient  # noqa: E402


WORLD_NAME = "world_reliquary_task5b"
DATAPACK_NAME = "reliquary_task5b"
APPLY_FUNCTION = "reliquary_task5b:apply"
APPLY_MARKER = "[RELIQUARY_TASK5B] apply complete"

# Positions from reliquary_task5a_converted_patch_1182.json
# (x, y, z, expected_block_id)
SUT_BLOCKS: dict[str, tuple[int, int, int, str]] = {
    # Altars
    "altar_inactive":     (300, 64, 300, "reliquary:alkahestry_altar"),
    "altar_active":       (302, 64, 300, "reliquary:alkahestry_altar"),
    # Cauldrons
    "cauldron_empty":     (304, 64, 300, "reliquary:apothecary_cauldron"),
    "cauldron_brewing":   (306, 64, 300, "reliquary:apothecary_cauldron"),
    "cauldron_splash":    (308, 64, 300, "reliquary:apothecary_cauldron"),
    "cauldron_partial":   (310, 64, 300, "reliquary:apothecary_cauldron"),
    # Mortars
    "mortar_empty":       (312, 64, 300, "reliquary:apothecary_mortar"),
    "mortar_vanilla":     (314, 64, 300, "reliquary:apothecary_mortar"),
    "mortar_mob_meta":    (316, 64, 300, "reliquary:apothecary_mortar"),
    "mortar_essence":     (318, 64, 300, "reliquary:apothecary_mortar"),
    "mortar_full":        (300, 64, 302, "reliquary:apothecary_mortar"),
    # Blocks without TE
    "fertile_lily_pad":   (302, 64, 302, "reliquary:fertile_lily_pad"),
    "interdiction_torch": (304, 64, 302, "reliquary:interdiction_torch"),
    "wraith_node":        (306, 64, 302, "reliquary:wraith_node"),
}
...
def install_server_properties(server_dir: Path) -> Path | None:
    ...
    backup = server_dir / "server.properties.before_reliquary_task6"
    if target.exists():
...
def run_server(server_dir: Path, label: str) -> tuple[subprocess.Popen, Path, Path]:
    ...
    out_log = server_dir / f"server_reliquary_task6_{label}_{stamp}_out.log"
    err_log = server_dir / f"server_reliquary_task6_{label}_{stamp}_err.log"
...
def write_markdown_report(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Reliquary Task 6 – Raport headless tick/restart",
        "",
        "## Podsumowanie",
        "",
        ...
        "- `reliquary_task6_headless_tick_restart_report.json`",
        f"- `headless_server/1.18.2/server_reliquary_task6_*_out.log`",
    ]
...
def write_handoff(report: dict[str, Any]) -> None:
    la = report.get("log_analysis", {})
    text = f"""# Handoff: Reliquary – Zadanie 6

## Podsumowanie sesji

Wykonano headless tick/restart verification dla `{WORLD_NAME}`. Runner uruchomił Forge 1.18.2,
wykonał funkcję datapacka `{APPLY_FUNCTION}`, sprawdził 14 skonwertowanych bloków, odczekał
...
- Apply marker found: {la.get('apply_marker_count', '?')}×
...
**Status:** {"✅ Zadanie 6 ukończone" if report.get("overall_pass") else "⚠️ Zadanie 6 z ostrzeżeniami"}
**Data:** {now_iso()[:10]}
"""
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    report: dict[str, Any] = {
        "name": "Reliquary Task 6 headless tick/restart",
        "status": "started",
        "start_time": now_iso(),
    ...
    try:
        ...
        print("[Reliquary Task6] Starting server first run...")
        ...
        print("[Reliquary Task6] Applying datapack...")
        prep = [
        ...
        print(f"[Reliquary Task6] Waiting {args.tick_seconds}s for ticks...")
        ...
        print("[Reliquary Task6] Stopping first run...")
        ...
        time.sleep(5)
        print("[Reliquary Task6] Restarting server...")
        ...
        print("[Reliquary Task6] Stopping restart run...")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Reliquary Task 6 headless tick/restart")
