...
## Zadanie 1. 
Wypisanie wszystkich bloków i tile entities dodawanych przez mod i opis tego co robią dane elementy bazując na źródłach z internetu oraz ze snippetami kodu z source code albo z dekompilowanego kodu (w zależności od tego co jest dostępne w folderze mod\_src) - korzystaj ze skilla: skills\analyze_and_summarize_sc
## Zadanie 2. 
...
## Zadanie 3. 
Napisanie kodu konwersji \- dla bloków i tile entities dla których działanie i metainformacje są trywialne i analogiczne dla wielu innych modów,  dla których jest prosta konwersja stwórz/wykorzystaj narzędzie w folderze w src, które ma obsługiwać przypadki ogólne między różnymi modami. Dla bardziej specyficznych konwersji dla danego moda rób skrypty w folderze w src z nazwą tego moda. Kod konwersji powinien uwzględniać przede wszystkim dynamiczne id elementów z głównej mapy, oraz dodatkowo dynamiczne id elementów z map testowych. Kod konwersji ma produkować eventy - jeden event to jest mapowanie jednego elementu na drugi - i te eventy muszą być kompatybilne z ogólnym handlerem wstawiającym dane elementy na podkładową mapę w wersji vanilla 1.18.2. Skorzystaj ze skilla: skills\coding_conversion
## Zadanie 4. 
Sprawdzenie dla stref głównej mapy (folder strefy, mapa w folderze mapa\_1710)czy kod pokrywa pełne \- bez jakichkolwiek edycji tej mapy. Sprawdzenie czy po konwersji symulacje przygotowane w punkcie 3 działałyby dla wersji 1.18.2 \- sprawdzenie zgodnie z kodem źródłowym modów na 1.18.2. Sprawdzenie czy konwertowane symulacje różnią się od symulacji 1.18.2 i analiza czy można to zostawić - podczas wykonywania zadania korzystaj ze skilla skills\main_map_coverage
## Zadanie 5A. 
Wykonanie testowej mapy w 1.7.10, na której wstawione zostaną wszystkie bloki i BE danego moda, a także różne kombinacje kilku BE z różnymi metainformacjami i stanami tych BE \- na przykład BE które maja możliwość przechowywania itemów mają być wstawione z różnymi itemami w środku. Dla BE które poz W przypadku wstawiania elementów do Sections w mca wykorzystaj skill: skills/mca-sections. Następnie wykonanie kodu konwersji na tej mapie. Dla modów należących do tej listy modów technicznych przygotuj dodatkowo testy integracyjne z redstone (do późniejszego odpalenia na headless serwer) opisane w skillu: skills\integration_test_with_redstone. Lista modów technicznych testowanych z redstone: Applied Energistics 2, Thermal Expansion, Thermal Dynamics, Thermal Foundation, Mekanism, Mekanism Generators, BuildCraft, Logistics Pipes, ProjectRed (Core/Integration itp.), ComputerCraft, IndustrialCraft 2, IC2 Nuclear Control, Big Reactors, Railcraft, PowerConverters, Extra Utilities, RFTools Builder, XNet, Integrated Dynamics, Create, Immersive Engineering, Pipez, Pretty Pipes
## Zadanie 5B. 
Wykonaj na headless serwerze vanilla 1.7.10 konwersji kopii mapy testowej do mapy vanilla, bez modów. Następnie przenieś przekonwertowaną mapę na headless serwer 
## Zadanie 6. 
Zrobienie testu na headless serwer z przekonwertowana mapą czy wyskakują jakieś błędy, sprawdzenie stanu mapy po jakimś czasie działania serwera i analiza jak zmienił się stan bloków i BE z konwertowanego moda po upływie jakiegoś czasu np 3 minut \- przy robieniu tych testów wykorzystaj skill: skills/autotest-on-server .
