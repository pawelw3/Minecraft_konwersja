...
description: Projektowanie i weryfikacja maszyn, pipelinów i systemów do wstawienia do Minecrafta (vanilla + mody). Anty-halucynacyjny workflow z wymaganiem źródeł dla każdej mechaniki.
disable-model-invocation: false
---

# Projektowanie i weryfikacja maszyn/pipelinów/systemów do Minecrafta

Ten skill prowadzi agenta tak, żeby **nie halucynował** zachowania gry, tylko:
...
## 8) Gdy brakuje źródeł: procedura "evidence by screenshots"

Czasem nie da się znaleźć wiarygodnego źródła (dokumentacji/kodu/changelogu/issue) opisującego **dokładnie** zachowanie konkretnego bloku/maszyny z moda. W takim przypadku **nie wolno zgadywać**.

### 8.1 Zasada
Jeśli po rozsądnym researchu (repo + wiki + issue + changelog) nadal nie masz pewności co do kluczowej reguły działania (throughput, tick rate, warunki aktywacji, zachowanie buforów), to **proponujesz użytkownikowi** zebranie dowodów w grze: **małe, kontrolowane przykłady + screeny**.

### 8.2 Jak poprosić użytkownika (szablon)
Poproś o:
...
### 8.3 Minimalne scenariusze testowe do poproszenia
- **Throughput test**: wejście stałe, wyjście do dużego bufora, mierz ilość po 60 s.
...
- **Chunk boundary/unload test** (jeśli pipeline jest duży).
- **Restart test**: exit to menu/reload świata.

### 8.4 Jak włączyć to do projektu
...
## 9) Definicja sukcesu tego skilla
