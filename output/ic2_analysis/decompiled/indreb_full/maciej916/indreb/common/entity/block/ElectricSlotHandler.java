...
public class ElectricSlotHandler extends SlotItemHandler {
   ...
   public ElectricSlotHandler(
      IItemHandler itemHandler, int index, int xPosition, int yPosition, boolean charging, InventorySlotType inventorySlotType, IEnergy energyStorage
   ) {
   ...
   public boolean m_5857_(@Nonnull ItemStack stack) {
      if (stack.m_41720_() instanceof IElectricItem est
         && (
            est.getEnergyTier().getLvl() > this.energyStorage.energyTier().getLvl()
               || est.getEnergyType() == EnergyType.RECEIVE && !this.charging
               || est.getEnergyType() == EnergyType.EXTRACT && this.charging
         )) {
         return false;
      }
      ...
      return switch (this.inventorySlotType) {
