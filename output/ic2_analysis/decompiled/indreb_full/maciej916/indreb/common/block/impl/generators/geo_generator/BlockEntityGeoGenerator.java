...
public class BlockEntityGeoGenerator extends IndRebBlockEntity implements ICooldown, IEnergyBlock, ITileSound {
   ...
   public FluidStorage fluidStorage = new FluidStorage(
      (Integer)ServerConfig.geo_generator_lava_capacity.get(), fluidStack -> fluidStack.getFluid() == Fluids.f_76195_
   );
   ...
   public BlockEntityGeoGenerator(BlockPos pWorldPosition, BlockState pBlockState) {
      ...
      this.createEnergyStorage(0, (Integer)ServerConfig.geo_generator_energy_capacity.get(), EnergyType.EXTRACT, EnergyTier.BASIC);
   }
   ...
   public void tickOperate(BlockState state) {
      ...
      if (this.cachedLava != this.fluidStorage.getFluidAmount()) {
         this.cachedLava = this.fluidStorage.getFluidAmount();
         updateState = true;
      }

      if (this.getCooldown() == 0) {
         if (this.getEnergyStorage().generateEnergy((Integer)ServerConfig.geo_generator_tick_generate.get(), true)
               == (Integer)ServerConfig.geo_generator_tick_generate.get()
            && this.fluidStorage.takeFluid(1, true) == 1) {
            this.fluidStorage.takeFluid(1, false);
            this.getEnergyStorage().generateEnergy((Integer)ServerConfig.geo_generator_tick_generate.get(), false);
            this.getEnergyStorage().updateGenerated((Integer)ServerConfig.geo_generator_tick_generate.get());
            this.active = true;
            updateState = true;
         }

         if (this.active
            && this.getEnergyStorage().generateEnergy((Integer)ServerConfig.geo_generator_tick_generate.get(), true)
               < (Integer)ServerConfig.geo_generator_tick_generate.get()
            && this.fluidStorage.takeFluid(1, true) == 1) {
            this.setCooldown(10);
         }
   ...
   public boolean isItemValidForSlot(int slot, @Nonnull ItemStack stack) {
      if (slot == 0) {
         ...
         if (cap != null) {
            return cap.getTanks() > 0 && cap.getFluidInTank(1).getFluid() == Fluids.f_76195_;
         }
      }
   ...
   public ItemStack insertItemForSlot(int slot, @Nonnull ItemStack stack, boolean simulate) {
      if (slot == 0) {
         IFluidHandlerItem cap = (IFluidHandlerItem)CapabilityUtil.getCapabilityHelper(stack, CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY).getValue();
         return cap != null && cap.getTanks() > 0 && cap.getFluidInTank(1).getFluid() == Fluids.f_76195_ ? null : stack;
      } else {
